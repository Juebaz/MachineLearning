...
if __name__ == "__main__":
    ...
    for n_neighbors, weights in itertools.product(n_neighbors_possible, weights_possible):
        ...
        for train_index, test_index in kf.split(X_train):
            ...
            temp = sum(classifieur_kpp.predict(X_validation)
                       == y_validation) / y_validation.size
            accuracy.append(temp)

        # On calcule l'erreur moyenne pour ce classifieur
        avgAccuracy = numpy.mean(accuracy)
        print("Number of neighbors: "+str(n_neighbors)+", Weight: " +
              str(weights)+", Mean accuray :"+str(avgAccuracy))
    ...
    for C, gamma in itertools.product(C_possible, gamma_possible):
        ...
        for train_index, test_index in kf.split(X_train):
            ...
            temp = sum(classifieur_svm.predict(X_validation)
                       == y_validation) / y_validation.size
            accuracy.append(temp)
        ...
        for hidden_layer_sizes, activation in itertools.product(hidden_layer_sizes_possible, activation_possible):
            ...
            for train_index, test_index in kf.split(X_train):
                ...
                temp = sum(classifieur_mlp.predict(X_validation)
                           == y_validation) / y_validation.size
                accuracy.append(temp)
            ...
            print("Hidden layer sizes : "+str(hidden_layer_sizes) +
                  ", Activation: "+str(activation)+", Mean accuray :"+str(avgAccuracy))
    ...
    print('Accuray for kNN on train: ' +
          str(model_final_kpp.score(X_train, y_train)))
    print('Accuray for SVM on train: ' +
          str(model_final_svm.score(X_train, y_train)))
    print('Accuray for MLP on train: ' +
          str(model_final_mlp.score(X_train, y_train)))
