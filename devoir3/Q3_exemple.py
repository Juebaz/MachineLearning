...
class DiscriminantANoyau:
    ...
    def get_h(self, X, x_t, y, params):
        ...
        for row in range(x_t.shape[0]):
            h[row] = numpy.sum(params[1:] * y * numpy.exp(-(numpy.linalg.norm(
                (X - x_t[row]), axis=1, ord=2)**2)/self.sigma**2)) + params[0]
        return h
    ...
    def fit(self, X, y):
        ...
        def evaluateFunc(hypers):
            ...
            obs_mal_classees = numpy.where(
                self.get_h(X, X, y_new, hypers) * y_new < 1)[0]
            x_mal_classe = X[obs_mal_classees]
            y_mal_classe = y_new[obs_mal_classees]

            # On trouve l'erreur
            err = numpy.sum((1 - y_mal_classe * self.get_h(X, x_mal_classe,
                                                           y, hypers))) + self.lambda_ * numpy.sum(hypers[1:])
            ...
            for row in range(grad.shape[0]-1):
              #                grad[row+1] = -numpy.sum(y[row] * y_mal_classe * numpy.exp(-cdist(x_mal_classe, X[row].reshape(1,-1), 'euclidean')**2/self.sigma**2)) + self.lambda_
                grad[row+1] = -numpy.sum(y[row] * y_mal_classe * numpy.exp(-(numpy.linalg.norm(
                    (x_mal_classe - X[row]), axis=1, ord=2)**2)/self.sigma**2)) + self.lambda_
...
if __name__ == "__main__":
    ...
    for sigma in sigma_possible:
        for lambda_ in lambda_possible:
            ...
            print("sigma : "+str(sigma)+", lambda: "+str(lambda_)+", Score train:" +
                  str(clf.score(X_train, y_train))+", Score test:"+str(clf.score(X_test, y_test)))
